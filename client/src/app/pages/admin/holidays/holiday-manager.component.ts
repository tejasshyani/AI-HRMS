...
@Component({
  ...
  template: `
    <div class="p-6 pb-12 space-y-6 max-w-7xl mx-auto">
      ...
      <!-- Loading State -->
      <div *ngIf="isLoading" class="card p-12 text-center border border-slate-200">
        <div class="loading-container">
          <div class="loader-spinner"></div>
          <div class="text-xs font-bold text-slate-700">Loading company holiday calendar...</div>
          <div class="text-[11px] text-slate-400">Fetching national, public, and company holidays</div>
        </div>
      </div>
      ...
      <div *ngIf="!isLoading" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <div *ngIf="holidays.length === 0" class="col-span-full card p-12 text-center text-slate-400">
          <i class="fa-regular fa-calendar-xmark text-3xl mb-2 text-slate-300"></i>
          <p>No holidays registered for this year.</p>
        </div>
      </div>
